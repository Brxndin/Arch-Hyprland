# Arch + Hyprland

Dotfiles das customizações que fiz ao usar Arch Linux com o Hyprland.

## Instalação

Usei `archinstall` com NetworkManager, btrfs, systemd-boot e type Minimal (então a maioria das coisas teve que ser baixada manualmente).

## Observações

- A estilização foi baseada no Glassmorphism.
- Em alguns arquivos os ícones podem parecer bugados pois estou usando a fonte JetBrainsMono Nerd.
- A maior parte das configurações foi feita pesquisando e usando o Gemini, mas alguns atalhos, pacotes, funcionalidades e a escolha do estilo foram pessoais.
- Algumas configurações e pacotes específicos foram usados por conta da minha GPU ser uma Nvidia RTX e por eu usar um adaptador Wi-Fi TP-Link. O Hyprland não lida tão bem com os drivers, então são necessárias algumas configurações manuais específicas e alguns pacotes a mais.

## Pacotes Usados

- amd-ucode 20260221-1
- base 3-3
- base-devel 1-2
- bat 0.26.1-2
- blueman 2.4.6-2
- bluez 5.86-4
- bluez-utils 5.86-4
- btop 1.4.6-1
- btrfs-progs 6.19-1
- capitaine-cursors 4-3
- cava 0.10.7-1
- cliphist 1:0.7.0-2
- code 1.109.5-1
- discord 1:0.0.127-1
- docker 1:29.2.1-1
- docker-compose 5.0.2-1
- drawing 1.0.2-2
- efibootmgr 18-3
- eza 0.23.4-3
- fastfetch 2.59.0-1
- ffmpegthumbnailer 2.3.0-1
- firefox 148.0-1
- git 2.53.0-1
- grim 1.5.0-2
- gst-plugin-pipewire 1:1.4.10-2
- gvfs 1.58.2-1
- hypridle 0.1.7-6
- hyprland 0.54.0-1
- hyprlock 0.9.2-8
- hyprpwcenter 0.1.2-1
- imagemagick 7.1.2.15-1
- imv 5.0.1-1
- iw 6.17-1
- kitty 0.45.0-4
- krita 5.2.16-1
- kvantum 1.1.6-1
- lib32-nvidia-utils 590.48.01-1
- lib32-vulkan-icd-loader 1.4.341.0-1
- libpulse 17.0+r98+gb096704c0-1
- libreoffice-fresh 26.2.1-1
- linux 6.18.13.arch1-1
- linux-firmware 20260221-1
- linux-headers 6.18.13.arch1-1
- mako 1.10.0-1
- mpv 1:0.41.0-3
- nano 8.7.1-1
- neovim 0.11.6-1
- network-manager-applet 1.36.0-1
- networkmanager 1.54.3-1
- noto-fonts-cjk 20240730-1
- noto-fonts-emoji 1:2.051-1
- nvidia-open-dkms 590.48.01-4
- nvidia-settings 590.48.01-1
- nvidia-utils 590.48.01-4
- nwg-look 1.0.6-1
- obs-studio 32.0.4-1
- openrgb 1.0rc2-5
- otf-font-awesome 7.2.0-1
- pacman-contrib 1.13.1-1
- papirus-icon-theme 20250501-1
- php 8.5.3-1
- pipewire 1:1.4.10-2
- pipewire-alsa 1:1.4.10-2
- pipewire-jack 1:1.4.10-2
- pipewire-pulse 1:1.4.10-2
- python-pywal 3.3.0-11
- qt5-wayland 5.15.18+kde+r55-1
- qt6-wayland 6.10.2-1
- qt6ct 0.11-4
- rofi 2.0.0-1
- rtl88x2bu-dkms-git 5.13.1.r212.825556e-1
- slurp 1.5.0-1
- snap-pac 3.0.1-3
- snapper 0.13.0-2
- speedtest-cli 2.1.3-10
- spotify-launcher 0.6.5-1
- starship 1.24.2-2
- steam 1.0.0.85-3
- swww 0.11.2-1
- thunar 4.20.7-1
- thunar-archive-plugin 0.6.0-1
- thunar-volman 4.20.0-2
- ttf-jetbrains-mono-nerd 3.4.0-2
- tumbler 4.20.1-1
- unzip 6.0-23
- usbutils 019-1
- vulkan-icd-loader 1.4.341.0-1
- waybar 0.15.0-2
- wireplumber 0.5.13-1
- wl-clipboard 1:2.2.1-3
- wob 0.16-2
- xdg-desktop-portal-hyprland 1.3.11-3
- yay 12.5.7-1
- yazi 26.1.22-2
- zed 0.225.10-1
- zip 3.0-11
- zram-generator 1.2.1-1
- zsh 5.9-5
